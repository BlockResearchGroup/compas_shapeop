...
def test_solver_basic_functionality():
    """Test that the DynamicSolver can perform basic solving operations without C++ errors."""
    ...
    original_positions = points_ref.copy()
    solver.solve(20)

    # Check that points have changed (solver should update points_ref in-place)
    assert not np.array_equal(original_positions, points_ref), "Points should change after solving"

    # Specifically check that at least some z-coordinates have changed due to the z-force
    z_changes = np.abs(points_ref[2, :] - original_positions[2, :])
