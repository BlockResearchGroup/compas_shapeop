...
class MeshSolver(Solver):
    ...
    def __init__(self, mesh):
        super().__init__()
        self.mesh = mesh
        self.points = mesh.to_vertices_and_faces()[0]
    ...
    def from_grid(cls, dx, nx, dy, ny):
        ...
        """
        mesh = Mesh.from_meshgrid(dx, nx, dy, ny).translated([dy * -0.5, dx * -0.5, 0])
        ms = cls(mesh)
        return ms
    ...
    def constrain_edge_lengths(self, min_range=0.99, max_range=1.01, shrink_factor=0, exclude_edges=None, weight=1e1):
        ...
        for u, v in self.mesh.edges():
            if (u, v) not in exclude_edges and (v, u) not in exclude_edges:
                if shrink_factor > 0:
                    cid = self.add_shrinking_edge_constraint(u, v, weight, shrink_factor)
    ...
    def constrain_face_diagonals(self, min_range=0.99, max_range=1.01, weight=1e1):
        ...
        for fkey in self.mesh.faces():
            ...
            if len(vertices) == 4:  # Only apply to quads
                cid = self.add_edge_strain_constraint(vertices[0], vertices[2], 0.2)
                constraint_ids.append(cid)
                cid = self.add_edge_strain_constraint(vertices[1], vertices[3], 0.2)
                constraint_ids.append(cid)
        return constraint_ids

    def constrain_face_planarity(self, type="plane", weight=1e5):
        ...
        faces = []
        for fkey in self.mesh.faces():
            face_vertices = list(self.mesh.face_vertices(fkey))
            faces.append(face_vertices)
            if type == "plane":
                cid = self.add_plane_constraint(face_vertices, weight)
            elif type == "circle":
                cid = self.add_circle_constraint(face_vertices, weight)
            constraint_ids.append(cid)
        return constraint_ids

    def constrain_face_regularization(self, weight=1e3):
        ...
        constraint_ids = []
        for fkey in self.mesh.faces():
            face_vertices = self.mesh.face_vertices(fkey)
            if len(face_vertices) > 3:  # Only apply to non-triangular faces
                cid = self.add_regular_polygon_constraint(face_vertices, weight)
                constraint_ids.append(cid)
        return constraint_ids

    def constrain_triface_bending(self, weight=1.0, min_range=1.0, max_range=1.0):
        ...
        for edge in self.mesh.edges():
            ...
            # connected faces
            faces = list(self.mesh.edge_faces(edge))
            if faces[0] is None or faces[1] is None:
                continue

            # Get vertices for both faces
            face1_vertices = self.mesh.face_vertices(faces[0])
            face2_vertices = self.mesh.face_vertices(faces[1])
    ...
    def fix_vertices(self, vertices=None, targets=None, weight=1e5):
        ...
        for idx, v in enumerate(vertices):
            target = targets[idx] if isinstance(targets, list) and len(targets) == len(vertices) else None
            cid = self.fix_vertex(v, target, weight)
            if cid is not None:
                constraint_ids.append(cid)

        return constraint_ids

    def fix_vertex(self, vertex=None, target=None, weight=1e5):
